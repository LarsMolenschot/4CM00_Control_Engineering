function fig_handling = mybode(f, H, C, fig_title, fig_handling)
    ...
    %=== Magnitude ===%
    fig_handling.plot = gobjects(1, size(H,2));
    for i = 1:size(H, 2)
      fig_handling.plot(1) = plot(fig_handling.H_tile, f(:,i), 20*log10(abs(H(:,i))));
    end
    set(fig_handling.H_tile, 'XScale', 'log'); grid(fig_handling.H_tile, 'on');
    ylabel(fig_handling.H_tile, '|H| [dB]','FontSize',24);
    title(fig_handling.H_tile, 'Magnitude','FontSize',24);

    %=== Phase ===%
    for i = 1:size(H, 2)
       fig_handling.plot(2) = plot(fig_handling.P_tile, f(:,i), rad2deg(angle(H(:,i))));
    end
    set(fig_handling.P_tile, 'XScale', 'log'); grid(fig_handling.P_tile, 'on');
    ylabel(fig_handling.P_tile, 'Phase [deg]','FontSize',24);
    title(fig_handling.P_tile, 'Phase','FontSize',24);
    ...
    if ~isempty(C)
        for i = 1:size(C, 2)
          fig_handling.C_plot(i) = plot(fig_handling.Coh_tile, f(:,ceil(i/2)), C(:,i));
        end
