...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Hardware_Performance_run_M) == (NULL)) &&
    !rtmGetStopRequested(Hardware_Performance_run_M);
  while (runModel) {
    sem_wait(&baserateTaskSem);
    pthread_mutex_lock(&rateTaskFcnRunningMutex[0]);
    Hardware_Performance_run_step();

    /* Get model outputs here */
    pthread_mutex_unlock(&rateTaskFcnRunningMutex[0]);
    stopRequested = !((rtmGetErrorStatus(Hardware_Performance_run_M) == (NULL)) &&
                      !rtmGetStopRequested(Hardware_Performance_run_M));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Hardware_Performance_run_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  Hardware_Performance_run_terminate();
  extmodeReset();
  sem_post(&stopSem);
  return NULL;
}
...
int main(int argc, char **argv)
{
  ...
  mwRaspiInit();
  MW_launchPyserver();
  rtmSetErrorStatus(Hardware_Performance_run_M, 0);
  ...
  /* Initialize model */
  Hardware_Performance_run_initialize();

  /* External Mode initialization */
  errorCode = extmodeInit(Hardware_Performance_run_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Hardware_Performance_run_M, true);
    }
  }
