...
{
  UNUSED(sig);
  rtmSetErrorStatus(Hardware_FRF_setup_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  /* Initialize model */
  Hardware_FRF_setup_initialize();

  /* External Mode initialization */
  errorCode = extmodeInit(Hardware_FRF_setup_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Hardware_FRF_setup_M, true);
    }
  }
