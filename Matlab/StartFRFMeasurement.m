function [d,y,u,e,r] = StartFRFMeasurement(p,N_samples,variance,measurementType, ModelName)
    ...
    val = (measurementType == 1);
    BlockPaths = find_system(ModelName);
    Index_switch = find(contains(BlockPaths,'Closed_loop')); % Get the correct string
    ...
    if length(Index_switch) > 1
        warning('There appear to be duplicate switch blocks in your model. Please remove all duplicate blocks.');
        Index_switch = Index_switch(1);
    end
    set_param(BlockPaths{Index_switch},'Value',num2str(val));
